...
public final class SemanticSearchNode implements FunctionNode {
     ...
     * {@link SemanticSearchNode} as arguments, surrounded by parentheses,
     * but formatted in the following way:
     * <ul>
     ...
     *     <li>The distance metric will be mapped to its corresponding string literal
     *     representation. For example, if the distance metric is 1, then the corresponding string
     *     literal would be {@code "COSINE"}.
     * </ul>
     * For example, the node
     * {@code SemanticSearchNode(0, -1.5f, 2, SearchSpec.EMBEDDING_SEARCH_METRIC_TYPE_COSINE)} will
     * look like `semanticSearch(getEmbeddingParameter(0), -1.5, 2, "COSINE")`
     *
     ...
     * }</pre>
     *
     * Then the query string will look like
