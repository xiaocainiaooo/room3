...
/**
 * Class responsible for coordination of requests to render into surfaces using OpenGL. This creates
 * a backing thread to handle EGL dependencies and draw leveraging OpenGL across multiple
 * [android.view.Surface] instances that can be attached and detached throughout the lifecycle of an
 * application. Usage of this class is recommended to be done on the UI thread.
 *
 * @param eglSpecFactory Callback invoked to determine the EGL spec version to use for EGL
 *   management. This is invoked on the GL Thread
 * @param eglConfigFactory Callback invoked to determine the appropriate EGLConfig used to create
...
class GLRenderer(
    ...
    /**
     * Collection of callbacks to be invoked when the EGL dependencies are initialized or torn down
     */
    private val mEglContextCallback = HashSet<EGLContextCallback>()

    /**
     * Removes the corresponding [RenderTarget] from management of the GLThread. This destroys the
     * EGLSurface associated with this surface and subsequent requests to render into the surface
     * with the provided token are ignored.
     *
     * If the [cancelPending] flag is set to true, any queued request to render that has not started
     * yet is cancelled. However, if this is invoked in the middle of the frame being rendered, it
     * will continue to process the current frame.
     *
     * Additionally if this flag is false, all pending requests to render will be processed before
     * the [RenderTarget] is detached.
     *
     ...
     * [android.view.Surface] instances through [attach] as well as schedule content to be drawn
    ...
    fun start(
        ...
        if (mGLThread == null) {
            ...
            mGLThread =
                GLThread(name, mEglSpecFactory, mEglConfigFactory).apply {
                    start()
                    if (!mEglContextCallback.isEmpty()) {
                        // Add a copy of the current collection as new entries to
                        // mEglContextCallback
    ...
    fun requestRender(target: RenderTarget, onRenderComplete: ((RenderTarget) -> Unit)? = null) {
        ...
        val callbackRunnable =
            if (onRenderComplete != null) {
                Runnable { onRenderComplete.invoke(target) }
            } else {
                null
            }
    ...
    /**
     * Resize the corresponding surface associated with the RenderTarget to the specified width and
     * height and re-render. This will destroy the EGLSurface created by
     * [RenderCallback.onSurfaceCreated] and invoke it again with the updated dimensions. An
     * optional callback is invoked on the backing thread after the resize operation is complete.
     *
    ...
    fun resize(
        ...
        val callbackRunnable =
            if (onResizeComplete != null) {
                Runnable { onResizeComplete.invoke(target) }
            } else {
                null
            }
     ...
     * [Runnable] does not block otherwise it can stall the GL thread. The EGLContext will be
     * created after [start] is invoked and before the runnable is executed.
     *
    ...
    /**
     * Stop the corresponding GL thread. This destroys all EGLSurfaces as well as any other EGL
     * dependencies. All queued requests that have not been processed yet are cancelled.
     *
    ...
    /**
     * Add an [EGLContextCallback] to receive callbacks for construction and destruction of EGL
     * dependencies.
     *
    ...
    /**
     * Remove [EGLContextCallback] to no longer receive callbacks for construction and destruction
     * of EGL dependencies.
     *
     * These callbacks are invoked on the backing thread
     */
    ...
    /**
     * Callbacks invoked when the GL dependencies are created and destroyed. These are logical
    ...
    interface EGLContextCallback {
         ...
         * guaranteed to be invoked before any instance of [RenderCallback.onSurfaceCreated] is
    ...
    /**
     * Interface used for creating an [EGLSurface] with a user defined configuration from the
    ...
    interface RenderCallback {
         ...
         * render into. The [EGLSurface] created here is guaranteed to be the current surface before
         * [onDrawFrame] is called. That is, implementations of onDrawFrame do not need to call
         * eglMakeCurrent on this [EGLSurface].
         *
         * This method is invoked on the GL thread.
         *
         * The default implementation will create a window surface with EGL_WIDTH and EGL_HEIGHT set
         * to [width] and [height] respectively. Implementations can override this method to provide
         * additional [EGLConfigAttributes] for this surface (ex. [EGL14.EGL_SINGLE_BUFFER].
         *
         * Implementations can return null to indicate the default surface should be used. This is
         * helpful in situations where content is to be rendered within a frame buffer object
         * instead of to an [EGLSurface]
         *
        ...
        /**
         * Callback used to issue OpenGL drawing commands into the [EGLSurface] created in
         * [onSurfaceCreated]. This [EGLSurface] is guaranteed to be current before this callback is
         * invoked and [EGLManager.swapAndFlushBuffers] will be invoked afterwards. If additional
         * scratch [EGLSurface]s are used here it is up to the implementation of this method to
         * ensure that the proper surfaces are made current and the appropriate swap buffers call is
         * made
         *
    ...
    /**
     * Adds the [android.view.Surface] to be managed by the GLThread. A corresponding [EGLSurface]
     * is created on the GLThread as well as a callback for rendering into the surface through
     * [RenderCallback]. Unlike the other [attach] methods that consume a [SurfaceView] or
     * [TextureView], this method does not handle any lifecycle callbacks associated with the target
     * surface. Therefore it is up to the consumer to properly setup/teardown resources associated
     * with this surface.
     *
     ...
     * @return [RenderTarget] used for subsequent requests to communicate with the provided Surface
     *   (ex. [requestRender] or [detach]).
    ...
    /**
     * Creates a new [RenderTarget] without a corresponding [android.view.Surface]. This avoids
     * creation of an [EGLSurface] which is useful in scenarios where only rendering to a frame
     * buffer object is required.
     *
     ...
     * backing thread.
     *
     * A corresponding [EGLSurface] is created on the GLThread as well as a callback for rendering
     ...
     * available. Similarly this surface will be detached from [GLRenderer] when the surface
     * provided by the [SurfaceView] is destroyed (i.e. [SurfaceHolder.Callback.surfaceDestroyed] is
     * called.
     *
     * If the [android.view.Surface] is already available by the time this method is invoked, it is
        ...
        if (thread != null) {
            ...
            val callback =
                object : SurfaceHolder.Callback2 {
                    ...
                    /**
                     * Optional condition that maybe used if we are issuing a blocking call to
                     * render in [SurfaceHolder.Callback2.surfaceRedrawNeeded] In this case we need
                     * to signal the condition of either the request to render has completed, or if
                     * the RenderTarget has been detached and the pending render request is
                     * cancelled.
                     */
                     ...
                     * [SurfaceHolder.Callback.surfaceDestroyed] In this case we need to signal the
                     * condition of either the request to detach has completed in case the
                     * GLRenderer has been forcefully stopped via [GLRenderer.stop] with the cancel
                    ...
                    val renderTarget =
                        RenderTarget(token, this@GLRenderer) @WorkerThread {
                    ...
                    override fun surfaceRedrawNeeded(p0: SurfaceHolder) {
                        ...
                        if (detachLatch.count > 0) {
                            ...
                            // surfaceRedrawNeeded is invoked on older API levels and is replaced
                            // with
                            // surfaceRedrawNeededAsync for newer API levels which is non-blocking
                            renderTarget.requestRender @WorkerThread { latch.countDown() }
                            latch.await()
                            renderLatch = null
                        }
                    }
                    ...
                    override fun surfaceDestroyed(holder: SurfaceHolder) {
                        ...
                        // [CountDownLatch] will signal when the [RenderTarget] detachment is
                        // complete
                        // or instantaneously if it was already detached
                        renderTarget.detachInternal(true) { onDetachComplete() }
                        detachLatch.await()
                    }
                }
     ...
     * [android.view.Surface] when the underlying [SurfaceTexture] is available. Similarly this
     * surface will be detached from [GLRenderer] if the underlying [SurfaceTexture] is destroyed
     * (i.e. [TextureView.SurfaceTextureListener.onSurfaceTextureDestroyed] is called.
     *
    ...
    fun attach(textureView: TextureView, renderer: RenderCallback): RenderTarget {
        ...
        if (thread != null) {
            ...
            val renderTarget =
                RenderTarget(token, this) @WorkerThread {
                    textureView.handler?.post { textureView.surfaceTextureListener = null }
                    detachLatch.countDown()
                }
            textureView.surfaceTextureListener =
                object : TextureView.SurfaceTextureListener {
                    ...
                    override fun onSurfaceTextureDestroyed(p0: SurfaceTexture): Boolean {
                        ...
                        // complete
                        // or instantaneously if it was already detached
                        renderTarget.detachInternal(true) { detachLatch.countDown() }
                        detachLatch.await()
                        return true
                    }
    ...
    internal constructor(
        ...
        /**
         * Request that this [RenderTarget] should have its contents redrawn. This consumes an
         * optional callback that is invoked on the backing thread when the rendering is completed.
         *
         ...
         *
         * @param onRenderComplete Optional callback called on the backing thread when rendering is
         *   finished
         */
        @JvmOverloads
        ...
        /**
         * Determines if the current RenderTarget is attached to GLRenderer. This is true until
         ...
         * EGLSurface created by [RenderCallback.onSurfaceCreated] and invoke it again with the
         * updated dimensions. An optional callback is invoked on the backing thread after the
        ...
        /**
         * Removes the corresponding [RenderTarget] from management of the GLThread. This destroys
         * the EGLSurface associated with this surface and subsequent requests to render into the
         * surface with the provided token are ignored.
         *
         * If the [cancelPending] flag is set to true, any queued request to render that has not
         * started yet is cancelled. However, if this is invoked in the middle of the frame being
         * rendered, it will continue to process the current frame.
         *
