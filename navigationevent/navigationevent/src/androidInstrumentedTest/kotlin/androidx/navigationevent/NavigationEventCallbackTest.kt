...
class NavigationEventCallbackTest {

    @Test
    fun remove_fromMultipleDispatchers_doesNotThrowConcurrentModificationException() {
        val callback = object : NavigationEventCallback(isEnabled = true) {}

        repeat(times = 5) {
            // Adds the same callback to multiple dispatchers. This setup verifies that
            // `NavigationEventCallback.remove()` handles these concurrent modifications to its
    ...
    fun remove_fromMultipleCloseables_doesNotThrowConcurrentModificationException() {
        ...
        repeat(times = 5) {
            ...
            // when closed. This setup verifies that `NavigationEventCallback.remove()` handles
            // these concurrent modifications to its internal state without throwing a
