...
public object RoomWarnings {
    /**
     * The warning dispatched by Room when the return value of a [Query] function does not exactly
     * match the columns in the query result.
     */
    public const val QUERY_MISMATCH: String = "ROOM_QUERY_MISMATCH"

    /**
     * The warning dispatched by Room when the object type declared in a [Query] function's multimap
     * return type does not implement `equals()` and `hashCode()`.
     */
    public const val DOES_NOT_IMPLEMENT_EQUALS_HASHCODE: String =
        "ROOM_TYPE_DOES_NOT_IMPLEMENT_EQUALS_HASHCODE"

    /**
     * Reported when Room cannot verify database queries during compilation due to lack of tmp dir
     * access in JVM.
     */
    ...
    /**
     * Reported when an [Entity] property that is annotated with [Embedded] has a sub property which
     * is annotated with [PrimaryKey] but the [PrimaryKey] is dropped while composing it into the
     * parent object.
     */
     ...
     */
    public const val INDEX_FROM_EMBEDDED_FIELD_IS_DROPPED: String = "ROOM_EMBEDDED_INDEX_IS_DROPPED"

    /**
     * Reported when an [Entity] that has a [Embedded] property whose type is another [Entity] and
     * that [Entity] has some indices defined. These indices will NOT be created in the containing
     * [Entity]. If you want to preserve them, you can re-define them in the containing [Entity].
     */
    public const val INDEX_FROM_EMBEDDED_ENTITY_IS_DROPPED: String =
        "ROOM_EMBEDDED_ENTITY_INDEX_IS_DROPPED"

    /**
     * Reported when an [Entity]'s parent declares an [Index]. Room does not automatically inherit
     * these indices to avoid hidden costs or unexpected constraints.
     *
     * If you want your child class to have the indices of the parent, you must re-declare them in
     * the child class. Alternatively, you can set [Entity.inheritSuperIndices] to `true`.
     */
    public const val INDEX_FROM_PARENT_IS_DROPPED: String = "ROOM_PARENT_INDEX_IS_DROPPED"

    /**
     * Reported when an [Entity] inherits a property from its super class and the property has a
     * [ColumnInfo] annotation with `index = true`.
     *
     * These indices are dropped for the [Entity] and you would need to re-declare them if you want
     * to keep them. Alternatively, you can set [Entity.inheritSuperIndices] to `true`.
     */
     ...
     * You can either set [Database.exportSchema] to `false` or provide `room.schemaLocation` to the
     * annotation processor. You are strongly advised to provide it and also commit them into your
    ...
    /**
     * Reported when a junction entity whose column is used in a [Relation] property with a
     * [Junction] does not contain an index. If the column is not covered by any index then a full
     * table scan might be performed when resolving the relationship.
     *
     ...
     * Room will pick that one by default but will print this warning in case the constructor choice
     * is important. You can always guide Room to use the right constructor using the `@Ignore`
     * annotation.
     */
    public const val DEFAULT_CONSTRUCTOR: String = "ROOM_DEFAULT_CONSTRUCTOR"

    /**
     * Reported when a [Query] function returns a data object that has relations but the function is
     * not annotated with [Transaction]. Relations are run as separate queries and if the query is
    ...
    /**
     * Reported when an [Entity] property's type do not exactly match the getter type. For instance,
     ...
     * `Foo.value` is `null` since Room will use the `getValue` function to read the value. So even
     * thought the database column is nullable, it will never be inserted as `null` if inserted as a
     * `Foo` instance.
     */
    public const val MISMATCHED_GETTER: String = "ROOM_MISMATCHED_GETTER_TYPE"

    /**
     * Reported when an [Entity] property's type do not exactly match the setter type. For instance,
     * in the following class:
     * ```
     * @Entity
     * class Foo {
     ...
     * ```
     *
     * If Room reads this entity from the database, it will always set `Foo.value` to `false` when
     * the column value is `null` since Room will use the `setValue` function to write the value
    ...
    /**
     * Reported when a nullable `Collection`, `Array` or `Optional` is returned from a DAO function.
