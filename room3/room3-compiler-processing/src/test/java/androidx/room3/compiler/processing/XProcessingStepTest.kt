...
class XProcessingStepTest {
    ...
    fun xProcessingStep() {
            ...
            object : XProcessingStep {
                override fun process(
                    ...
                    elementsByAnnotation[OtherAnnotation::class.qualifiedName]
                        ...
                        ?.forEach { annotatedElements[OtherAnnotation::class] = it.qualifiedName }
                    elementsByAnnotation[MainAnnotation::class.qualifiedName]
                        ?.filterIsInstance<XTypeElement>()
                        ?.forEach { annotatedElements[MainAnnotation::class] = it.qualifiedName }
                    return emptySet()
                }
        ...
        assertAbout(JavaSourcesSubjectFactory.javaSources())
            .that(listOf(main, other))
            .processedWith(mainProcessor)
            .compilesWithoutError()
        assertThat(annotatedElements)
        ...
        val processingStep =
            object : XProcessingStep {
                override fun process(
                        ...
                        ?.forEach {
                            ...
                            val spec =
                                TypeSpec.classBuilder(className)
                                    .addAnnotation(
                                        AnnotationSpec.builder(OtherAnnotation::class.java)
                                            .apply { addMember("value", "\"foo\"") }
                                            .build()
                                    )
                                    .build()
                    ...
                    elementsByAnnotation[OtherAnnotation::class.qualifiedName]
                        ?.filterIsInstance<XTypeElement>()
                        ?.forEach { otherAnnotatedElements.add(it.type.toString()) }
                    return emptySet()
                }
    ...
    fun cachingBetweenSteps() {
        ...
        val otherStep =
            object : XProcessingStep {
                override fun annotations(): Set<String> =
                    setOf(OtherAnnotation::class.qualifiedName!!)
    ...
    fun kspReturnsUnprocessed() {
        ...
        val processingStep =
            object : XProcessingStep {
                override fun process(
                    ...
                    return elementsByAnnotation.values.flatten().toSet().also { returned = it }
                }
        ...
        // processor name is 'null' because it is a local anonymous class
        assertThat(result.diagnostics[Diagnostic.Kind.ERROR]?.map { it.msg } ?: emptyList<String>())
            .containsExactly(
                "null was unable to process 'foo.bar.Other' because not all of its dependencies " +
    ...
    fun javacAnnotatedElementsByStep() {
        ...
        val mainStep =
            object : XProcessingStep {
                ...
                override fun process(
                    ...
                    elementsByStep[this] =
                        elementsByAnnotation.values.flatten().map {
                            (it as XTypeElement).qualifiedName
                        }
                    return emptySet()
                }
            }
        ...
        assertAbout(JavaSourcesSubjectFactory.javaSources())
            ...
            .compilesWithoutError()
        assertThat(elementsByStep[mainStep]).containsExactly("foo.bar.Main")
        assertThat(elementsByStep[otherStep]).containsExactly("foo.bar.Other")
    }
    ...
    fun javacDeferredViaException() {
        ...
        val mainStep =
            object : XProcessingStep {
                ...
                override fun process(
                    ...
                    try {
                        val otherElement =
                            env.requireTypeElement(
                                mainElement
                                    .requireAnnotation(MainAnnotation::class.asJClassName())
                        ...
                        val generatedType =
                            otherElement
                                .requireAnnotation(SingleTypeValueAnnotation::class.asJClassName())
                                .getAsType("value")
        ...
        assertAbout(JavaSourcesSubjectFactory.javaSources())
            .that(listOf(main, anotherSource))
            .processedWith(
                object : JavacBasicAnnotationProcessor() {
                    override fun processingSteps(): List<XProcessingStep> {
                        return listOf(mainStep)
            ...
            object : XProcessingStep {
                ...
                override fun process(
                    ...
                    return if (round++ == 0) {
                        ...
                        val className = ClassName.get("foo.bar", "Main_Impl")
                        val spec = TypeSpec.classBuilder(className).build()
                        JavaFile.builder(className.packageName(), spec).build().writeTo(env.filer)
                        elementsByAnnotation.values.flatten().toSet()
                    } else {
                        emptySet()
                    }
    ...
    fun starSupportedAnnotation_multiRound() {
        ...
        runProcessorTest(sources = listOf(kotlinSrc), createProcessingSteps = { listOf(step) }) {}
        // 3 for each backend, 1st initial round, 2nd round due to new gen sources, 3rd round over
        assertThat(invocations)
            .isEqualTo(mapOf(XProcessingEnv.Backend.JAVAC to 3, XProcessingEnv.Backend.KSP to 3))
    }
    ...
    fun validateElement() {
        ...
        assertAbout(JavaSourcesSubjectFactory.javaSources())
            .that(listOf(main, someAnnotation))
            .processedWith(
                object : JavacBasicAnnotationProcessor() {
                    override fun processingSteps() = listOf(processingStep)
                }
