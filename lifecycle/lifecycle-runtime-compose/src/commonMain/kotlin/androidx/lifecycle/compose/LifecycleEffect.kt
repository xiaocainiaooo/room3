 ...
 *
 * Using a [LifecycleEventObserver] to listen for when [LifecycleEventEffect] enters the
 * composition, [onEvent] will be launched when receiving the specified [event].
 *
 ...
 * of storing callback data as a [Lifecycle.State] in a [MutableState]. Instead, see
 * [currentStateAsState] to obtain a [State<Lifecycle.State>][State] that may be used to launch jobs
 ...
 * [Lifecycle.Event.ON_START] or [Lifecycle.Event.ON_STOP] (or any new unique value of [key1]). The
 * ON_START effect will be the body of the [effects] block and the ON_STOP effect will be within the
 ...
 * [onStopOrDispose][LifecycleStartStopEffectScope.onStopOrDispose] clause as the final statement in
 * its [effects] block. If your operation does not require an effect for _both_
 * [Lifecycle.Event.ON_START] and [Lifecycle.Event.ON_STOP], a [LifecycleEventEffect] should be used
 * instead.
 *
 * A [LifecycleStartEffect]'s _key_ is a value that defines the identity of the effect. If the key
 * changes, the [LifecycleStartEffect] must [dispose][LifecycleStartStopEffectScope.onStopOrDispose]
 * its current [effects] and reset by calling [effects] again. Examples of keys include:
 ...
 *
 * This function uses a [LifecycleEventObserver] to listen for when [LifecycleStartEffect] enters
 * the composition and the effects will be launched when receiving a [Lifecycle.Event.ON_START] or
 * [Lifecycle.Event.ON_STOP] event, respectively. If the [LifecycleStartEffect] leaves the
 * composition prior to receiving an [Lifecycle.Event.ON_STOP] event,
 * [onStopOrDispose][LifecycleStartStopEffectScope.onStopOrDispose] will be called to clean up the
 * work that was kicked off in the ON_START effect.
 *
 * This function should **not** be used to launch tasks in response to callback events by way of
 * storing callback data as a [Lifecycle.State] in a [MutableState]. Instead, see
 * [currentStateAsState] to obtain a [State<Lifecycle.State>][State] that may be used to launch jobs
 * in response to state changes.
 *
...
/**
 * Schedule a pair of effects to run when the [Lifecycle] receives either a
 * [Lifecycle.Event.ON_START] or [Lifecycle.Event.ON_STOP] (or any new unique value of [key1] or
 * [key2]). The ON_START effect will be the body of the [effects] block and the ON_STOP effect will
 * be within the (onStopOrDispose clause)[LifecycleStartStopEffectScope.onStopOrDispose]:
 * ```
 * LifecycleStartEffect(lifecycleOwner) {
 *     // add ON_START effect here
 *
 ...
 * instead.
 *
 * A [LifecycleStartEffect]'s _key_ is a value that defines the identity of the effect. If a key
 * changes, the [LifecycleStartEffect] must [dispose][LifecycleStartStopEffectScope.onStopOrDispose]
 * its current [effects] and reset by calling [effects] again. Examples of keys include:
 * * Observable objects that the effect subscribes to
 * * Unique request parameters to an operation that must cancel and retry if those parameters change
 ...
 * Schedule a pair of effects to run when the [Lifecycle] receives either a
 * [Lifecycle.Event.ON_START] or [Lifecycle.Event.ON_STOP] (or any new unique value of [key1] or
 * [key2] or [key3]). The ON_START effect will be the body of the [effects] block and the ON_STOP
 * effect will be within the (onStopOrDispose
 * clause)[LifecycleStartStopEffectScope.onStopOrDispose]:
 * ```
 * LifecycleStartEffect(lifecycleOwner) {
 *     // add ON_START effect here
 *
...
/**
 * Schedule a pair of effects to run when the [Lifecycle] receives either a
 * [Lifecycle.Event.ON_START] or [Lifecycle.Event.ON_STOP] (or any new unique value of [keys]). The
 * ON_START effect will be the body of the [effects] block and the ON_STOP effect will be within the
 * (onStopOrDispose clause)[LifecycleStartStopEffectScope.onStopOrDispose]:
 * ```
 * LifecycleStartEffect(lifecycleOwner) {
...
/**
 * Interface used for [LifecycleStartEffect] to run the effect within the onStopOrDispose clause
...
/**
 * Receiver scope for [LifecycleStartEffect] that offers the [onStopOrDispose] clause to couple the
 * ON_START effect. This should be the last statement in any call to [LifecycleStartEffect].
 *
...
/**
 * Schedule a pair of effects to run when the [Lifecycle] receives either a
 * [Lifecycle.Event.ON_RESUME] or [Lifecycle.Event.ON_PAUSE] (or any new unique value of [key1]).
 * The ON_RESUME effect will be the body of the [effects] block and the ON_PAUSE effect will be
 * within the (onPauseOrDispose clause)[LifecycleResumePauseEffectScope.onPauseOrDispose]:
 * ```
 ...
 * [onPauseOrDispose][LifecycleResumePauseEffectScope.onPauseOrDispose] clause as the final
 * statement in its [effects] block. If your operation does not require an effect for _both_
 * [Lifecycle.Event.ON_RESUME] and [Lifecycle.Event.ON_PAUSE], a [LifecycleEventEffect] should be
 * used instead.
 *
 * A [LifecycleResumeEffect]'s _key_ is a value that defines the identity of the effect. If a key
 * changes, the [LifecycleResumeEffect] must
 * [dispose][LifecycleResumePauseEffectScope.onPauseOrDispose] its current [effects] and reset by
 * calling [effects] again. Examples of keys include:
 * * Observable objects that the effect subscribes to
 * * Unique request parameters to an operation that must cancel and retry if those parameters change
 *
 * This function uses a [LifecycleEventObserver] to listen for when [LifecycleResumeEffect] enters
 * the composition and the effects will be launched when receiving a [Lifecycle.Event.ON_RESUME] or
 * [Lifecycle.Event.ON_PAUSE] event, respectively. If the [LifecycleResumeEffect] leaves the
 * composition prior to receiving an [Lifecycle.Event.ON_PAUSE] event,
 * [onPauseOrDispose][LifecycleResumePauseEffectScope.onPauseOrDispose] will be called to clean up
 * the work that was kicked off in the ON_RESUME effect.
 *
 * This function should **not** be used to launch tasks in response to callback events by way of
 * storing callback data as a [Lifecycle.State] in a [MutableState]. Instead, see
 * [currentStateAsState] to obtain a [State<Lifecycle.State>][State] that may be used to launch jobs
...
/**
 * Schedule a pair of effects to run when the [Lifecycle] receives either a
 * [Lifecycle.Event.ON_RESUME] or [Lifecycle.Event.ON_PAUSE] (or any new unique value of [key1] or
 * [key2]). The ON_RESUME effect will be the body of the [effects] block and the ON_PAUSE effect
 * will be within the (onPauseOrDispose clause)[LifecycleResumePauseEffectScope.onPauseOrDispose]:
 * ```
 * LifecycleResumeEffect(lifecycleOwner) {
 *     // add ON_RESUME effect here
 *
 ...
 * Schedule a pair of effects to run when the [Lifecycle] receives either a
 * [Lifecycle.Event.ON_RESUME] or [Lifecycle.Event.ON_PAUSE] (or any new unique value of [key1] or
 * [key2] or [key3]). The ON_RESUME effect will be the body of the [effects] block and the ON_PAUSE
 * effect will be within the (onPauseOrDispose
 * clause)[LifecycleResumePauseEffectScope.onPauseOrDispose]:
 * ```
 * LifecycleResumeEffect(lifecycleOwner) {
 *     // add ON_RESUME effect here
 *
...
/**
 * Schedule a pair of effects to run when the [Lifecycle] receives either a
 * [Lifecycle.Event.ON_RESUME] or [Lifecycle.Event.ON_PAUSE] (or any new unique value of [keys]).
 * The ON_RESUME effect will be the body of the [effects] block and the ON_PAUSE effect will be
 * within the (onPauseOrDispose clause)[LifecycleResumePauseEffectScope.onPauseOrDispose]:
 * ```
 * LifecycleResumeEffect(lifecycleOwner) {
...
/**
 * Interface used for [LifecycleResumeEffect] to run the effect within the onPauseOrDispose clause
...
/**
 * Receiver scope for [LifecycleResumeEffect] that offers the [onPauseOrDispose] clause to couple
 * the ON_RESUME effect. This should be the last statement in any call to [LifecycleResumeEffect].
 *
 * This scope is also a [LifecycleOwner] to allow access to the
