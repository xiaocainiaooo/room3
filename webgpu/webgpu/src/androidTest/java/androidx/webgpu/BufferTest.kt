...
class BufferTest {
    ...
    fun testBufferSizeAndUsage() {
        ...
        val buffer =
            device.createBuffer(GPUBufferDescriptor(usage = bufferUsage, size = bufferSize))
    ...
    fun testDestroy() {
        val buffer =
            device.createBuffer(GPUBufferDescriptor(usage = BufferUsage.MapRead, size = 16))

        buffer.destroy()

        // After destroying the buffer, further operations should fail.
        assertThrows(ValidationException::class.java) {
            runBlocking { buffer.mapAndAwait(MapMode.Read, 0, 16) }
        }
    }
    ...
    fun testWriteAndReadBuffer() {
        ...
        val cpuDataBuffer =
            ByteBuffer.allocateDirect(bufferSize.toInt()).order(ByteOrder.nativeOrder()).apply {
                asFloatBuffer().put(cpuData).rewind()
            }
    ...
    fun testMapAsyncUnmap() {
        ...
        assertThrows(Error::class.java) { buffer.getMappedRange(0, bufferSize) }
    }
    ...
    fun testReadMappedRangeSucceedsAndDataIsCorrect() {
        ...
        val byteBuffer =
            ByteBuffer.allocateDirect(bufferSize.toInt()).order(ByteOrder.nativeOrder()).apply {
                asFloatBuffer().put(initialData).rewind()
            }
        ...
        runBlocking {
            val unusedBufferMapReturn =
                buffer.mapAndAwait(MapMode.Read, offset = 0, size = bufferSize)
        }
    ...
    fun testWriteMappedRangeFailsWhenNotMapped() {
        val buffer =
            device.createBuffer(GPUBufferDescriptor(size = 16, usage = BufferUsage.MapWrite))
        val byteBuffer = ByteBuffer.allocateDirect(16)

        assertThrows(WebGpuException::class.java) { buffer.writeMappedRange(0, byteBuffer) }
    }
