...
class ShaderModuleTest {
    ...
    private suspend fun getCompilationInfo(code: String): CompilationInfo {
        val shaderModule =
            device.createShaderModule(
                ShaderModuleDescriptor(shaderSourceWGSL = ShaderSourceWGSL(code))
            )
    ...
    /**
     * Verifies that an invalid shader correctly produces a validation error and a compilation error
     * message.
     */
    @Test
    ...
    /**
     * Verifies that the primary compilation error for the invalid shader is reported on the correct
     * line number.
     */
    @Test
    ...
    /**
     * Verifies that the absolute character offset reported in the error message is mathematically
     * consistent with its reported line number and line position (column).
     */
    @Test
