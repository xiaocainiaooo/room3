...
class TextureTest {
    ...
    /**
     * Verifies that creating a texture with an invalid combination of usage flags (e.g.,
     * multisampling with storage binding) fails validation.
     */
    @Test
    fun createTexture_withInvalidUsageCombination_fails() {
        ...
        val invalidDescriptor =
            TextureDescriptor(
                ...
                usage =
                    TextureUsage.RenderAttachment or
                        TextureUsage.StorageBinding, // Invalid combination
            )
    ...
    /**
     * Verifies that using a view from a destroyed texture generates an uncaptured error upon
    ...
    fun createBitmap_withInvalidWidth_fails() {
        ...
        assertThrows(DawnException::class.java) { runBlocking { texture.createBitmap(device) } }
    }
}
