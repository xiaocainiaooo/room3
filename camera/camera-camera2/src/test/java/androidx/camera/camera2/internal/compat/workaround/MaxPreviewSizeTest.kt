...
class MaxPreviewSizeTest {
    ...
    fun largerThanDefaultPreviewResolution_returnsPrivResolution() {
        ...
        assertThat(mMaxPreviewSize.getMaxPreviewResolution(smallSize))
            .isEqualTo(SELECT_RESOLUTION_PRIV)
    }

    @Test
    fun smallerThanDefaultResolution_returnsDefaultResolution() {
        val largeDefaultSize = Size(10001, 10000)
        assertThat(mMaxPreviewSize.getMaxPreviewResolution(largeDefaultSize))
            .isEqualTo(largeDefaultSize)
    }
    ...
    fun noResolutionQuirk_returnsOriginalMaxPreviewResolution() {
        noQuirk_returnsOriginalMaxPreviewResolution(Mockito.mock(ExtraCroppingQuirk::class.java))
    }

    private fun noQuirk_returnsOriginalMaxPreviewResolution(quirk: ExtraCroppingQuirk?) {
        val maxPreviewSize = MaxPreviewSize(quirk)
        val result = maxPreviewSize.getMaxPreviewResolution(SELECT_RESOLUTION_JPEG)
