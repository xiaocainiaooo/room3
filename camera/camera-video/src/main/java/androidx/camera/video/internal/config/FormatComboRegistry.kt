...
private constructor(private val formatComboMapping: Map<Int, Map<String?, Set<FormatCombo>>>) {
     ...
     *
     * @param outputFormat The specific container ID or [OUTPUT_FORMAT_AUTO] for any.
     * @param videoMime The specific video MIME or [VideoSpec.MIME_TYPE_AUTO] for any.
     * @param audioMime The specific audio MIME or [AudioSpec.MIME_TYPE_AUTO] for any.
    ...
    public fun getCombos(
        ...
        val containersToSearch =
            if (outputFormat == OUTPUT_FORMAT_AUTO) {
        ...
        for (container in containersToSearch) {
            ...
            val videoMimesToSearch =
                if (videoMime == VideoSpec.MIME_TYPE_AUTO) {
                    videoMap.keys
                } else {
                    listOf(videoMime)
                }
            ...
            for (videoMime in videoMimesToSearch) {
                ...
                if (audioMime == AudioSpec.MIME_TYPE_AUTO) {
