...
sealed class PerfettoConfig(internal val isTextProto: Boolean) {
     ...
     *
     * Note: this format is not recommended for long term use - the [Binary] proto representation is
     * more likely to remain stable over time, across Perfetto/Android OS versions. For more
     * information, see
...
private fun ftraceDataSource(atraceApps: List<String>) =
    TraceConfig.DataSource(
        config =
            DataSourceConfig(
                name = "linux.ftrace",
                ...
                ftrace_config =
                    FtraceConfig(
                        ftrace_events =
                            listOf(
                                // We need to do process tracking to ensure kernel ftrace events
                                // targeted at short-lived
...
private fun processStatsDataSource(
    stackSamplingConfig: StackSamplingConfig?
): TraceConfig.DataSource {
            ...
            DataSourceConfig(
                ...
                process_stats_config =
                    ProcessStatsConfig(
                        ...
                        // This flag appears to be unreliable on API 29 unbundled perfetto, so to
                        // avoid very
...
private val ANDROID_POWER_DATASOURCE =
    TraceConfig.DataSource(
        config =
            DataSourceConfig(
                name = "android.power",
